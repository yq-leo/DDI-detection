...
class KGCN(BaseModel):
    ...
    def build(self):
        ...
        # get receptive field
        #receptive_list = Lambda(lambda x: self.get_receptive_field(x), name='receptive_filed')(input_drug_two)
        ...
        drug1_embed, drug2_embed = neigh_ent_embed_list_drug_one[0], neigh_ent_embed_list[0]
        drug_embed = Concatenate()([drug1_embed, drug2_embed])
        drug_reshaped = Reshape((drug_embed.shape[-1], -1))(drug_embed)
        drug_conv_out = Conv1D(16, kernel_size = 3, padding = 'same', activation = 'relu', input_shape = (64, 1))(drug_reshaped)
        drug_drop_out = Dropout(0.2, input_shape = (drug_conv_out.shape[1], drug_conv_out.shape[2]))(drug_conv_out)
        drug_maxpool_out = MaxPooling1D(2)(drug_drop_out)
        drug_flattened = Flatten()(drug_maxpool_out)
        drug_result = Dense(2)(Dense(64, activation = 'relu')(drug_flattened))
    ...
    def score(self, x, y, threshold=0.5):
        ...
        auc = m.roc_auc_score(y_true=y_true, y_score=y_pred)
        p, r, t = m.precision_recall_curve(y_true=y_true, probas_pred=y_pred)
        aupr = m.auc(r, p)
        y_pred = [1 if prob >= threshold else 0 for prob in y_pred]
        acc = m.accuracy_score(y_true=y_true, y_pred=y_pred)
        f1 = m.f1_score(y_true=y_true, y_pred=y_pred)
        precision = m.precision_score(y_true, y_pred)
        recall = m.recall_score(y_true, y_pred)
        mcc = m.matthews_corrcoef(y_true, y_pred)

        return auc, acc, f1, aupr, precision, recall, mcc
