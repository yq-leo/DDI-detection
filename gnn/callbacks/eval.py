...
class KGCNMetric(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        precisions, recalls, _thresholds = precision_recall_curve(y_true = y_true, probas_pred = y_pred)
        aupr=m.auc(recalls, precisions)
        ...
        save_logs = logs.copy()
        save_logs['dataset'] = self.dataset
        save_logs['aggregator_type'] = self.aggregator_type
        save_logs['kfold'] = self.k
        save_logs['epoch_count'] = epoch + 1
        print(f'Logging Info - epoch: {epoch+1}, val_auc: {auc}, val_aupr: {aupr}, val_acc: {acc}, val_f1: {f1}')
        write_log('log/train_history.txt', save_logs, mode='a')
